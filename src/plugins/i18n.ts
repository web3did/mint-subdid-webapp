...
import { ILanguageOption, LANGUAGE, LanguageOptions } from '~/constant/language'
...
export default async ({ app, store, req }: Context, inject: Function): Promise<void> => {
  const lang: ILanguageOption = process.server ? resolveLanguageFromRequest(req) : store.getters[COMMON_KEYS.computedLanguage]
